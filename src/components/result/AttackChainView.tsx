    ...
    <section>
      ...
      <div className="mt-8">
        {analysis.attackChain.map((stage, i) => {
          const style = STAGE_STYLE[stage.stage] ?? { color: "#8b93a3", label: stage.stage };
      ...
      {analysis.manipulationTactics.length > 0 && (
        <div className="mt-4 rounded-lg border border-red/30 bg-red/5 p-5">
          ...
          <p className="mt-2 text-[14px] leading-relaxed text-text-primary/90">
            This message combines {analysis.manipulationTactics.map((t) => t.name.toLowerCase()).join(", ")} to
            prevent careful verification before you act.
          </p>
        </div>
      )}
    </section>
