import { AnalyzeUrlSchema, jsonError } from "@/lib/validation";
...
export async function POST(req: Request) {
  ...
  try {
    const analysis = await runAnalysis({ type: "url", rawText: parsed.data.url, submittedUrl: parsed.data.url });
    return Response.json({ analysis });
  } catch {
    return jsonError(500, "ANALYSIS_FAILED", "Something went wrong while analyzing this URL. Please try again.");
  }
}
